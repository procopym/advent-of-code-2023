...
for (let i = 0; i < games.length - 1; i++) {
  ...
  const reveals = gameResults.split("; ");
  let isPossible = true;
  const minimal = new Map();
  for (let j = 0; j < reveals.length; j++) {
    const counts = new Map();
    const values = reveals[j].trim().split(",");

    for (let k = 0; k < values.length; k++) {
      const [value, key] = values[k].trim().split(" ");
      if (counts.has(key)) {
  ...
  let power = 1;
  for (let [_, value] of minimal) {
    power = power * value;
  }

  gamesPower.push(power);
