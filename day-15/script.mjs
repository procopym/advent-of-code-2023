...
function findIndex(box, label) {
  return box.findIndex((x) => {
    const [itemLabel] = x;
...
function removeLabelFromTheBox(box, label) {
  const index = findIndex(box, label);
  if (index < 0) {
    return;
  }

  for (let i = index; i < box.length - 1; i++) {
    box[i] = box[i + 1];
  }
...
function addOrUpdateLens(box, label, length) {
  const index = findIndex(box, label);

  if (index < 0) {
    box.push([label, +length]);
  } else {
    box[index] = [label, +length];
  }
}
...
for (let i = 0; i < sequences.length; i++) {
  ...
  const id = hash(sequence);
  ans += id; // part 1

  if (sequence.endsWith("-")) {
    const [label] = sequence.split("-");
    const labelHash = hash(label);
    removeLabelFromTheBox(boxes[labelHash], label);
  } else {
    const [label, length] = sequence.split("=");
    const labelHash = hash(label);
    addOrUpdateLens(boxes[labelHash], label, length);
  }
}
